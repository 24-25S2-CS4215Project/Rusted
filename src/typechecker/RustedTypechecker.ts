...
class ParentRef extends AbstractTypeClosure {
  constructor(
    public parent: CompileTimeEnvironment,
  ) {
...
export class RustedTypeChecker extends RustedVisitor<string> {
  ...
  private isIntegerType(type: string): boolean {
    return type === "i32" || type === "i64" || type === "u32" || type === "u64" || type === "usize";
  }
  ...
  private extractIdentifierFromExpr(expr: Assignment_exprContext): string {
    ...
    const logicalExpr = expr.logical_expr();
    return logicalExpr.comparison_expr(0).additive_expr(0).multiplicative_expr(0).unary_expr(0).ref_primary_expr()!.primary_expr()!.IDENTIFIER()!.getText();
  }
  ...
  private declareVariable(name: string, type: string, mutable: boolean, immutableBorrow: number = 0, mutableBorrow: number = 0): void {
    ...
    this.env.extend(name, new TypeClosure(type, mutable, true, immutableBorrow, mutableBorrow));
  }
  ...
  visitFunction = (ctx: FunctionContext): string => {
    ...
    if (blockType !== "void" && blockType !== "()" && !this.areTypesCompatible(returnType, blockType)) {
      this.addError(`Function '${functionName}' declares return type '${returnType}' but returns '${blockType}'`);
    }
  ...
  visitLet_statement = (ctx: Let_statementContext): string => {
    ...
    if (ctx.expression()) {
      ...
      if (!this.areTypesCompatible(varType, exprType)) {
        this.addError(`Cannot assign value of type '${exprType}' to variable '${varName}' of type '${varType}'`);
      }
      ...
      if (this.isExpressionSimpleIdentifier(ctx.expression()!)) {
        ...
        if (rhsVar && rhsVar.dropped && this.shouldTransferOwnership(rhsVar.type)) {
  ...
  visitReturn_statement = (ctx: Return_statementContext): string => {
    ...
    if (this.currentFunctionReturnType && !this.areTypesCompatible(this.currentFunctionReturnType, returnType)) {
      this.addError(`Return type '${returnType}' doesn't match function return type '${this.currentFunctionReturnType}'`);
    }
  ...
  visitIf_statement = (ctx: If_statementContext): string => {
    ...
    if (this.areTypesCompatible(ifBlockType, elseBlockType) && ifBlockType !== "void" && elseBlockType !== "void") {
      return ifBlockType;
    }
  ...
  visitAssignment_expr = (ctx: Assignment_exprContext): string => {
    ...
    if (ctx.assignment_expr()) {
      ...
      if (leftExpr.comparison_expr().length === 1 &&
        leftExpr.comparison_expr(0).additive_expr().length === 1 &&
        leftExpr.comparison_expr(0).additive_expr(0).multiplicative_expr().length === 1 &&
        leftExpr.comparison_expr(0).additive_expr(0).multiplicative_expr(0).unary_expr(0).ref_primary_expr() &&
        leftExpr.comparison_expr(0).additive_expr(0).multiplicative_expr(0).unary_expr(0).ref_primary_expr()!.primary_expr() &&
        leftExpr.comparison_expr(0).additive_expr(0).multiplicative_expr(0).unary_expr(0).ref_primary_expr()!.primary_expr()!.IDENTIFIER()) {

        const varName = leftExpr.comparison_expr(0).additive_expr(0).multiplicative_expr(0).unary_expr(0).ref_primary_expr()!.primary_expr()!.IDENTIFIER()!.getText();
        ...
        if (variable) {
          ...
          if (!this.areTypesCompatible(variable.type, rightType)) {
            this.addError(`Cannot assign value of type '${rightType}' to variable '${varName}' of type '${variable.type}'`);
          }
  ...
  visitLogical_expr = (ctx: Logical_exprContext): string => {
    ...
    for (let i = 0; i < ctx.comparison_expr().length; i++) {
      ...
      if (!this.isBooleanType(exprType)) {
        this.addError(`Logical operator expected boolean operand, got '${exprType}'`);
      }
    }
  ...
  visitComparison_expr = (ctx: Comparison_exprContext): string => {
    ...
    for (let i = 1; i < ctx.additive_expr().length; i++) {
      ...
      if (!this.areTypesCompatible(leftType, rightType)) {
        this.addError(`Cannot compare values of types '${leftType}' and '${rightType}'`);
      }
    }
  ...
  visitAdditive_expr = (ctx: Additive_exprContext): string => {
    ...
    for (let i = 1; i < ctx.multiplicative_expr().length; i++) {
      ...
      if (!(
        ...
        this.addError(`Operator '${operator}' not defined for types '${leftType}' and '${rightType}'`);
      }
    }
  ...
  visitUnary_expr = (ctx: Unary_exprContext): string => {
    ...
    if (operator === "-" && !this.isIntegerType(operand)) {
      this.addError(`Unary operator '-' requires numeric operand, got '${operand}'`);
      return "i32"; // Default to integer type
    } else if (operator === "!" && !this.isBooleanType(operand)) {
      this.addError(`Unary operator '!' requires boolean operand, got '${operand}'`);
      return "bool";
    }
    ...
    if (ctx.getText().startsWith("&")) {
      ...
      if (isMut && ctx.primary_expr()?.IDENTIFIER()) {
        ...
        if (variable && !variable.mutable) {
          this.addError(`Cannot take mutable reference to immutable variable '${varName}'`);
        }
      }
  ...
  visitFunction_call = (ctx: Function_callContext): string => {
    ...
    if (paramTypes.length !== argCount) {
      this.addError(`Function '${funcName}' expects ${paramTypes.length} arguments, got ${argCount}`);
    }
    ...
    for (let i = 0; i < argCount; i++) {
      ...
      if (this.isExpressionSimpleIdentifier(argExpr)) {
        ...
        // Check if the variable is already dropped
        if (argVar && argVar.dropped && this.shouldTransferOwnership(argVar.type)) {
          this.addError(`Cannot use moved value: '${argVarName}' as function argument`);
        }
      ...
      if (!this.areTypesCompatible(paramType, argType) && paramType !== "any") {
        this.addError(`Argument ${i + 1} of '${funcName}' expects type '${paramType}', got '${argType}'`);
      }
    }
